...
meanstd<-grep("*mean\\(\\)|*std\\(\\)", names(data), ignore.case=TRUE)
    ##excludes measuremend on MeanFreq() and gravityMean or where Mean is used as part of additional function
data_step2<-data[,sort(meanstd)]

###Adds participant and activity information with descriptive variable names###
...
###Creates and saves tidy data as .txt file
